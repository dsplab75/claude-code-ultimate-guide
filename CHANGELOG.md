...
- **Link checker added to main guide repo** (`.github/workflows/link-check.yml`): Monthly cron (1st of month, 08:00 UTC) scanning `guide/**/*.md` and `examples/**/*.md`. Creates a GitHub issue with `bug` + `broken-links` labels when broken links are detected. Accepts 200, 202, 206, 302, 403, 429; excludes mailto, anchors, file://, social media, localhost, and template placeholder patterns.
